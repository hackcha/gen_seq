...
class Generator(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
        ...
        self.ones = tf.constant([1.0] * self.batch_size, dtype=tf.float32)
        ...
        #根据输入的数据格式，在句子之前加了start_token符号。
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            gen_o = gen_o.write(i+1, tf.reduce_sum(tf.multiply(tf.one_hot(next_token, self.num_emb, 1.0, 0.0),
                                                             tf.nn.softmax(o_t)), 1))  # [batch_size] , prob
            gen_x = gen_x.write(i+1, next_token)  # indices, batch_size
            return i + 1, x_tp1, h_t, gen_o, gen_x

        # gen_o.write(0, )
        gen_o = gen_o.write(0, self.ones)#开始的字符固定使用_GO
        gen_x = gen_x.write(0, self.start_token)
        _, _, _, self.gen_o, self.gen_x = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3, _4: i+1 < self.sequence_length,
            body=_g_recurrence,
