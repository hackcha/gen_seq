...
import keras
from keras.layers import Embedding,Masking, Lambda
from keras.layers import Dense, Input, Flatten, merge, BatchNormalization
...
class Discriminator_RNN(object):
    """
    A CNN for text classification.
    Uses an embedding layer, followed by a convolutional, max-pooling and softmax layer.
    """
    ...
    def __init__(
        ...
        with tf.variable_scope('discriminator'):
            ...
            with tf.name_scope("output"):
                ...
                l2_loss += tf.nn.l2_loss(W)
                l2_loss += tf.nn.l2_loss(b)
                self.scores = tf.nn.xw_plus_b(self.encoded_f, W, b, name="scores")
                self.ypred_for_auc = tf.nn.softmax(self.scores)
                self.predictions = tf.argmax(self.scores, 1, name="predictions")
